...
class BaseTrainer(object):
    ...
    def train(self, resume=False):
        ...
        while epoch < self.cfg.SOLVER.EPOCHS + 1:
            ...
            if self.cfg.distributation:
                self.train_loader.sampler.set_epoch(epoch)

            self.train_epoch(epoch)
            end = datetime.datetime.now()
            print((end - begin).total_seconds() / 3600)
    ...
    def train_iter(self, data, epoch, train_meters):
        ...
        # # forward
        # d=self.distiller(image=image, target=target, epoch=epoch)
        # print(len(d))
        preds, losses_dict = self.distiller(image=image, target=target, epoch=epoch)

        # backward
        loss = sum([l.mean() for l in losses_dict.values()])
        # loss=sum(losses_dict['loss_kd'].mean(),losses_dict['loss_ce'].mean())
        loss.backward()
        nn.utils.clip_grad_norm_(self.distiller.parameters(), max_norm=40, norm_type=2)
